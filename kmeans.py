...
StandardKMeans2: the native kmeans algorithm using distance 1
...
class mp1KMeans:
    ...
    def centers_update(self, X, labels):
        new_centers = np.array([self.low_prec(self.low_prec(X[labels == i]).mean(axis=0)) for i in range(self.n_clusters)])
        return new_centers
